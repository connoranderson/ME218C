...
#define STRAIGHT_CONVERSION 0.55
#define TURN_CONVERSION 0.80
#define MAX_DUTY 70
#define MAX_BACKWARD_DUTY 90
...
void UpdateSteering(int8_t StraightMeasure, int8_t TurnMeasure){
	...
	if(DutyCycleLeftMotor < -MAX_BACKWARD_DUTY) {
		DutyCycleLeftMotor = -MAX_BACKWARD_DUTY;
	}
	if(DutyCycleRightMotor < -MAX_BACKWARD_DUTY) {
		DutyCycleRightMotor = -MAX_BACKWARD_DUTY;
	}
	printf("LeftDuty: %d",DutyCycleLeftMotor);
	printf("RightDuty: %d",DutyCycleRightMotor);
...
static void WriteToMotors(int8_t driveSignal_LeftMotor,int8_t driveSignal_RightMotor)
{
	...
	if(rightMotorStatus == 0x00){ // Going backwards
		SetDutyCycleRightMotor(-driveSignal_RightMotor);   // If going backwards, drive signal will be negative. Subtract from max duty for backwards PWM.
//		printf("\r\n Duty Cycle Right Motor (Backwards): %d", -driveSignal_RightMotor);
	}else{
		SetDutyCycleRightMotor(FULL_DUTY - driveSignal_RightMotor);
//		printf("\r\n Duty Cycle Right Motor (Forward): %d", FULL_DUTY - driveSignal_RightMotor);
	}
	
	// Set left motor speed
	if(leftMotorStatus == 0x00){ // Going backwards
			SetDutyCycleLeftMotor(-driveSignal_LeftMotor);     // If going backwards, drive signal will be negative. Subtract from max duty for backwards PWM.
//			printf("\r\n Duty Cycle Left Motor (Backwards): %d", -driveSignal_RightMotor);
	}else{
		SetDutyCycleLeftMotor(FULL_DUTY - driveSignal_LeftMotor);
//		printf("\r\n Duty Cycle Left Motor (Forward): %d", FULL_DUTY - driveSignal_LeftMotor);
	} 
