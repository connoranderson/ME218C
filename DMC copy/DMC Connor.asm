...
;	BSF		LATA, LATA4
	...
	;If not paired, turn off the LEDs and make sure the timer is off.
	BTFSC	IS_PAIRED, 0
	RETURN



	;Turning the LEDS off, 	stopping the timer.
	;banksel LATA
	;BCF		LATA, RED_LED
	;BCF		LATA, BLUE_LED
	;banksel T2CON
...
START_MOVING:
		...
		;If the motor is already moving, queue the event to be performed once it's done.
		;BTFSC		MOTOR_STATE, 0
...
PWM_GO_HI: 	; Go to the next look up table value
	...
	; ORIGINAL CODE
		...
		;	movwf		testvar
		;	 movf		index, W
   		;	 call 		DIM_LED_ARRAY      	; Call the step table
		;	subwf		testvar, w
