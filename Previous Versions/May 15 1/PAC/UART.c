...
// Addresses for all relevant XBEEs
#define RECEIVE_ADDRESS 2189
#define DESTINATION_ADDRESS 2111
...
//#define NUM_FRAMES 20 // Determines how many outgoing bytes we will send every time
//#define RETURN_PACKET_NUM_FRAMES 7 // How many frames come back after we've sent a message
#define RECEIVE_LENGTH_LSB 2 // Location of LSB of length bytes in incoming message

static uint8_t *outgoingMessage; // First entry in array
//static uint32_t outgoingMessage;
//static uint32_t incomingMessage;
static uint8_t *incomingMessage;
				...
				*(incomingMessage + rxIndex) = incomingByte; // Put new byte in LSB
				...
				if(rxIndex == incomingMessageSize - 1){    // Handles the final bit to make sure index gets reset.
					...
					*(incomingMessage + rxIndex) = incomingByte; // Put new byte in LSB
					// Reset index counter
					rxIndex = 0;
					printf("Successful message receive");
					// See if message has success in status byte
