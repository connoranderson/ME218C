...
static uint16_t DestinationAddress = 0x2189;
static uint8_t messagearr[8];
static uint8_t *message = &messagearr[0];
    ...
    case InitPState :       // If current state is initial Psedudo State
        if ( ThisEvent.EventType == ES_INIT ) // only respond to ES_Init
        {
            //Prep the timer hardware.
						...
						Steering_HWInit();					// Initialize the steering systems
						printf("Init Packets \r\n");					
...
static void TransmitStatusPacket(uint8_t PairedStatus) { 
	...
	TransmitMessage(StatusPacket, STATUS_PACKET_SIZE, DestinationAddress);
}
...
****************************************************************************/
static void ExecuteControl(void) {
	int8_t FWD_BCK = (int8_t) *(message+1);
	 printf("\r\n FWD/BCKWD: %d ", FWD_BCK);
	int8_t LFT_RGHT = (int8_t) *(message+2);
	 printf("\r\n LFT/RGHT: %d ",LFT_RGHT);
	 printf("\r\n Special Action: %d ", *(message+3));
	int8_t SP_ACT = *(message+3);
	if(SP_ACT && BIT1HI != 0) {
