    ...
    if ( ThisEvent.EventType == ES_NEW_KEY) // there was a key pressed
    {
        switch ( toupper(ThisEvent.EventParam))
        {
          // this sample is just a dummy so it posts a ES_NO_EVENT
            case 'P' : ThisEvent.EventType = PAIR_REQUEST; 
            break;
            case 'K' : ThisEvent.EventType = KEY_RECEIVED;
            break;
            case 'C' : ThisEvent.EventType = COMMAND_RECEIVED;
            break;
            case 'G' : ThisEvent.EventType = ES_TIMEOUT;
