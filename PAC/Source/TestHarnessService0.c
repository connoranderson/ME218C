...
static int8_t Forward = 0x00;
static int8_t Turning = 0x00;
static uint8_t SpecialAction = 0x00;
...
//      printf("\r Beginning Test Program %d\r\n", MyPriority);
      break;
...
//          //puts("Pulsed!\r");
			 Turning = 127; 
			 SetTurn(Turning);
				printf("RGHT CMD sent\r\n");
      }
			if( 'u' == ThisEvent.EventParam ){
				ES_Event UnpairEvent;
				UnpairEvent.EventType = MANUAL_UNPAIR;
				PostPACSM(UnpairEvent);
				SpecialAction = 0x01;
				SetSpecialAction(SpecialAction);
				printf("Manual unpair CMD sent\r\n");
      }
			if( 'o' == ThisEvent.EventParam ){
				SpecialAction = BIT7HI;
				SetSpecialAction(SpecialAction);
				printf("Turn on special action\r\n");
      }
			if( 'p' == ThisEvent.EventParam ){
				SpecialAction = 0x00;
				SetSpecialAction(SpecialAction);
				printf("Turn off special action\r\n");
      }
      break;
    default :
      break;
