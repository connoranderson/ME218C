...
#define HALF_ROLL_OFFSET 30
#define CENTER_UPPER_BOUND 5
#define CENTER_LOWER_BOUND -5
...
bool InitMyoInputs(void) {
    ...
    // Make bits 2-7 on Port B to be inputs
...
int8_t GetGesture(void) {
  ...
  // Read the necesary pins for reading the gesture and save them into the Gesture byte
	Gesture = (HWREG(GPIO_PORTA_BASE+(GPIO_O_DATA+ALL_BITS)) & GESTURE_BITS) >> 6; //Must shift so that the constants match.
...
int8_t GetRoll(void) {
  ...
  //int8_t SplitRoll = RawRoll-MAX_RAW_ROLL/2;              // Initialize split role to make the conversion easier so that the role is split evenly bewteen positive and negative values
  //int8_t TurnMeasure = SPEED*SplitRoll/(RawRoll/2); // Initialize variable int8_t TurnMeasure and set it based on Roll
  int8_t SplitRoll = RawRoll - HALF_ROLL_OFFSET; // Reading the data bits without shift causes the roll bucket number to be multiplied by 4.
	int8_t TurnMeasure;
	if((SplitRoll < CENTER_UPPER_BOUND) && (SplitRoll > CENTER_LOWER_BOUND)){
		TurnMeasure = 0;
	} else {
		 TurnMeasure = SplitRoll * 4;
	}
	TurnMeasure *= -1; //Controls in the protocol are flipped from the Pi buckets.
	return TurnMeasure;
}
